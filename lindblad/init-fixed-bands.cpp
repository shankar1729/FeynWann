...
{	
	...
	const int bandStart, bandStop, nBandsActive;
	const bool bandSelectAll; //!< whether to require all bands in range, or any, in k-point selection
	...
	LindbladInit(FeynWann& fw, const vector3<int>& NkFine,
		double Emin, double Emax, int bandStart, int bandStop, bool bandSelectAll,
		bool ePhEnabled, double ePhDelta, bool defectEnabled)
	: fw(fw), NkFine(NkFine), nkTot(NkFine[0]*NkFine[1]*NkFine[2]),
		Emin(Emin), Emax(Emax), bandStart(bandStart), bandStop(bandStop),
		nBandsActive(bandStop - bandStart), bandSelectAll(bandSelectAll),
	...
	{	int nActiveCur = 0;
		...
		if(bandSelectAll ? (nActiveCur == nBandsActive) : (nActiveCur > 0))
		{	nActiveTot += nBandsActive;
			k.push_back(state.k);
	...
	void kpointSelect(const std::vector<vector3<>>& k0)
	{
		...
		if(not nActiveTot)
			die("No k-points or bands selected: check compatibility of energy and band ranges.\n");
	...
	{	//Find pairs of momentum conserving electron states with this q:
		...
		{	const vector3<>& k1 = k[ik1];
			...
			const double *E1begin = E.data()+ik1*fw.nBands+bandStart, *E1end = E1begin+nBandsActive;
			const double *E2begin = E.data()+ik2*fw.nBands+bandStart, *E2end = E2begin+nBandsActive;
			//--- check energy ranges:
	...
	{	LindbladFile::Kpoint& kp = kpOffset[state.ik];
		...
		const double *Ebegin = state.E.data();
		int innerOffset = bandStart;
		int outerOffset = bandStart;
		kp.innerStart = 0;
		kp.nInner = nBandsActive;
		kp.nOuter = nBandsActive;
	...
	{	const FeynWann::StateE& e1 = *(mEph.e1);
		...
		#define PREP(i) \
			size_t ik##i; \
			int innerOffset##i = bandStart; \
			int nInner##i = nBandsActive; \
	...
	void saveData(const std::vector<vector3<>>& k0, string outFile)
	{
		...
		h.dmuMax = Emax;
		h.Tmax = INFINITY;
		h.pumpOmegaMax = INFINITY;
		h.probeOmegaMax = INFINITY;
		h.nk = k.size();
		h.nkTot = nkTot;
...
{	
	InitParams ip = FeynWann::initialize(argc, argv, "Initialize sparse matrices for Lindblad dynamics (fixed bands version)");
	...
	//--- energy range
	const double Emin = inputMap.get("Emin") * eV; //lower energy cutoff for active space in eV
	const double Emax = inputMap.get("Emax") * eV; //upper energy cutoff for active space in eV
	//--- which bands to use
	const int bandStart = inputMap.get("bandStart"); //starting band index of active window from within Wannier set (0-based)
	const int bandStop = inputMap.get("bandStop"); //ending band index of active window from within Wannier set (not included)
	const bool bandSelectAll = inputMap.getBool("bandSelectAll"); //whether all bands need to be in range, or any.
	//--- pump
	...
	logPrintf("bandSelectAll = %s\n", bandSelectAll ? "yes" : "no");
	logPrintf("ePhMode = %s\n", ePhMode.c_str());
	...
	LindbladInit lb(fw, NkFine, Emin, Emax, bandStart, bandStop, bandSelectAll, ePhEnabled, ePhDelta, defectEnabled);
