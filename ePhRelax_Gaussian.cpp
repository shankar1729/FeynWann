...
{
	Interp1 dos, dosPh;
	diagMatrix f0, dfPert; //Initial Fermi and photon-perturbation to distribution
	...
	diagMatrix fdot(double t, const diagMatrix& f) const
...
{	const ePhRelax& e = *((ePhRelax*)params);
	...
	diagMatrix fdotMat = e.fdot(t, fMat); //calculate result in diagMatrix form
...
//double t; // define global variable for time
...
{	ePhRelax e(argc, argv);
	...
	{	int status = gsl_odeiv2_driver_apply(odeDriver, &t, t+e.dt, f.data());
		...
		logPrintf("%d %5g  %19.13le  %19.13le  %19.13le  %7.2lf  %.1f%%\n", j,t/fs, dEe/Eunits, dEl/Eunits, (dEe+dEl)/Eunits, f.back()/Kelvin, 100.*t/e.tMax);
		logFlush();
		j++;
	}
	logPrintf("fArr.size() : %d\n",int(fArr.size()));
	logPrintf("done.\n"); logFlush();
